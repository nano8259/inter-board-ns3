...
#ifndef TIPC_SIGNAL_LINK_TX_BUFFER_H
#define TIPC_SIGNAL_LINK_TX_BUFFER_H

#include "ns3/object.h"
#include "ns3/traced-value.h"
#include "ns3/sequence-number.h"
#include "ns3/tcp-option-sack.h"
#include "tipc-signal-link-tx-item.h"
 ...
 * class TipcSignalLinkTxItem: instead of storing a list of packets, we store a list of
 * TipcSignalLinkTxItem. Each item has different flags (check the corresponding
 ...
 */
class TipcSignalLinkTxBuffer : public Object
{
public:
  /**
   ...
   */
  TipcSignalLinkTxBuffer (uint32_t n = 0);
  virtual ~TipcSignalLinkTxBuffer (void);
   ...
   * \returns a pointer to the TipcSignalLinkTxItem that corresponds to what requested.
   * Please do not delete the pointer, nor modify Packet data or sequence numbers.
   */
  TipcSignalLinkTxItem* CopyFromSequence (uint32_t numBytes, const SequenceNumber32& seq);
  ...
  void DiscardUpTo (const SequenceNumber32& seq,
                    const Callback<void, TipcSignalLinkTxItem *> &beforeDelCb = m_nullCb);
  ...
  uint32_t Update (const TcpOptionSack::SackList &list,
                   const Callback<void, TipcSignalLinkTxItem *> &sackedCb = m_nullCb);
...
private:
  friend std::ostream & operator<< (std::ostream & os, TipcSignalLinkTxBuffer const & tcpTxBuf);

  typedef std::list<TipcSignalLinkTxItem*> PacketList; //!< container for data stored in the buffer
   ...
   */
  void RemoveFromCounts (TipcSignalLinkTxItem *item, uint32_t size);
   ...
   */
  TipcSignalLinkTxItem* GetNewSegment (uint32_t numBytes);
   ...
   */
  TipcSignalLinkTxItem* GetTransmittedSegment (uint32_t numBytes, const SequenceNumber32 &seq);
   ...
   * \return the item that contains the right packet
   */
  TipcSignalLinkTxItem* GetPacketFromList (PacketList &list, const SequenceNumber32 &startingSeq,
                                uint32_t numBytes, const SequenceNumber32 &requestedSeq,
                                bool *listEdited = nullptr) const;

  /**
   * \brief Merge two TipcSignalLinkTxItem
   *
   ...
   */
  void MergeItems (TipcSignalLinkTxItem *t1, TipcSignalLinkTxItem *t2) const;

  /**
   * \brief Split one TipcSignalLinkTxItem
   *
   ...
   */
  void SplitItems (TipcSignalLinkTxItem *t1, TipcSignalLinkTxItem *t2, uint32_t size) const;
  ...
  static Callback<void, TipcSignalLinkTxItem *> m_nullCb; //!< Null callback for an item
};
 ...
 */
std::ostream & operator<< (std::ostream & os, TipcSignalLinkTxBuffer const & tcpTxBuf);
 ...
 * \returns The output stream.
 */
std::ostream & operator<< (std::ostream & os, TipcSignalLinkTxItem const & item);

} // namespace ns3

#endif /* TIPC_SIGNAL_LINK_TX_BUFFER_H */
