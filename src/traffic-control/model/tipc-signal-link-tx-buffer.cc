...
NS_LOG_COMPONENT_DEFINE ("TipcSignalLinkTxBuffer");
NS_OBJECT_ENSURE_REGISTERED (TipcSignalLinkTxBuffer);

Callback<void, TipcSignalLinkTxItem *> TipcSignalLinkTxBuffer::m_nullCb = MakeNullCallback<void, TipcSignalLinkTxItem*> ();
TypeId
TipcSignalLinkTxBuffer::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::TipcSignalLinkTxBuffer")
    .SetParent<Object> ()
    .SetGroupName ("Tipc")
    .AddConstructor<TipcSignalLinkTxBuffer> ()
    .AddTraceSource ("UnackSequence",
                     "First unacknowledged sequence number (SND.UNA)",
                     MakeTraceSourceAccessor (&TipcSignalLinkTxBuffer::m_firstByteSeq),
 ...
 * SndBufSize is passed to TipcSignalLinkTxBuffer by TcpSocketBase::SetSndBufSize() and in
 * turn, TipcSignalLinkTxBuffer:SetMaxBufferSize(). Therefore, the m_maxBuffer value
 * initialized below is insignificant.
 */
TipcSignalLinkTxBuffer::TipcSignalLinkTxBuffer (uint32_t n)
  : m_maxBuffer (32768), m_size (0), m_sentSize (0), m_firstByteSeq (n)
{
  m_rWndCallback = MakeNullCallback<uint32_t> ();
}

TipcSignalLinkTxBuffer::~TipcSignalLinkTxBuffer (void)
{
  PacketList::iterator it;

  for (it = m_sentList.begin (); it != m_sentList.end (); ++it)
    {
      TipcSignalLinkTxItem *item = *it;
...
SequenceNumber32
TipcSignalLinkTxBuffer::HeadSequence (void) const
{
  return m_firstByteSeq;
}

SequenceNumber32
TipcSignalLinkTxBuffer::TailSequence (void) const
{
  return m_firstByteSeq + SequenceNumber32 (m_size);
}

uint32_t
TipcSignalLinkTxBuffer::Size (void) const
{
  return m_size;
}

uint32_t
TipcSignalLinkTxBuffer::MaxBufferSize (void) const
{
  return m_maxBuffer;
}

void
TipcSignalLinkTxBuffer::SetMaxBufferSize (uint32_t n)
{
  m_maxBuffer = n;
}

bool
TipcSignalLinkTxBuffer::IsSackEnabled (void) const
{
  return m_sackEnabled;
}

void
TipcSignalLinkTxBuffer::SetSackEnabled (bool enabled)
{
  m_sackEnabled = enabled;
}

uint32_t
TipcSignalLinkTxBuffer::Available (void) const
{
  return m_maxBuffer - m_size;
}

void
TipcSignalLinkTxBuffer::SetDupAckThresh (uint32_t dupAckThresh)
{
  m_dupAckThresh = dupAckThresh;
}

void
TipcSignalLinkTxBuffer::SetSegmentSize (uint32_t segmentSize)
{
  m_segmentSize = segmentSize;
}

uint32_t
TipcSignalLinkTxBuffer::GetRetransmitsCount (void) const
{
  return m_retrans;
}

uint32_t
TipcSignalLinkTxBuffer::GetLost (void) const
{
  return m_lostOut;
}

uint32_t
TipcSignalLinkTxBuffer::GetSacked (void) const
{
  return m_sackedOut;
}

void
TipcSignalLinkTxBuffer::SetHeadSequence (const SequenceNumber32& seq)
{
...
bool
TipcSignalLinkTxBuffer::Add (Ptr<Packet> p)
{
  ...
  if (p->GetSize () <= Available ())
    {
      if (p->GetSize () > 0)
        {
          TipcSignalLinkTxItem *item = new TipcSignalLinkTxItem ();
...
uint32_t
TipcSignalLinkTxBuffer::SizeFromSequence (const SequenceNumber32& seq) const
{
  NS_LOG_FUNCTION (this << seq);
...
TipcSignalLinkTxItem *
TipcSignalLinkTxBuffer::CopyFromSequence (uint32_t numBytes, const SequenceNumber32& seq)
{
  ...
  TipcSignalLinkTxItem *outItem = nullptr;
...
TipcSignalLinkTxItem*
TipcSignalLinkTxBuffer::GetNewSegment (uint32_t numBytes)
{
  ...
  TipcSignalLinkTxItem *item = GetPacketFromList (m_appList, startOfAppList,
...
TipcSignalLinkTxItem*
TipcSignalLinkTxBuffer::GetTransmittedSegment (uint32_t numBytes, const SequenceNumber32 &seq)
{
  NS_LOG_FUNCTION (this << numBytes << seq);
  ...
  TipcSignalLinkTxItem *item = GetPacketFromList (m_sentList, m_firstByteSeq, s, seq, &listEdited);
...
std::pair <TipcSignalLinkTxBuffer::PacketList::const_iterator, SequenceNumber32>
TipcSignalLinkTxBuffer::FindHighestSacked () const
{
  ...
  for (auto it = m_sentList.begin (); it != m_sentList.end (); ++it)
    {
      const TipcSignalLinkTxItem *item = *it;
      if (item->m_sacked)
        {
...
void
TipcSignalLinkTxBuffer::SplitItems (TipcSignalLinkTxItem *t1, TipcSignalLinkTxItem *t2, uint32_t size) const
{
...
TipcSignalLinkTxItem*
TipcSignalLinkTxBuffer::GetPacketFromList (PacketList &list, const SequenceNumber32 &listStartFrom,
                                ...
                                bool *listEdited) const
{
  ...
  TipcSignalLinkTxItem *currentItem = nullptr;
  TipcSignalLinkTxItem *outItem = nullptr;
  ...
  while (it != list.end ())
    {
      ...
      if (seq < beginOfCurrentPacket + currentPacket->GetSize ())
        {
          ...
          if (seq == beginOfCurrentPacket)
          ...
          else if (seq > beginOfCurrentPacket)
            {
              ...
              TipcSignalLinkTxItem *firstPart = new TipcSignalLinkTxItem ();
      ...
      if (seq + numBytes <= beginOfCurrentPacket + currentPacket->GetSize ())
          ...
          if (numBytes == currentPacket->GetSize ())
            {
              ...
              if (currentItem->m_packet == outItem->m_packet)
              ...
              else
                {
                  ...
                  TipcSignalLinkTxItem *previous = *(--it);
      ...
      else
        {
          ...
          TipcSignalLinkTxItem *next = (*it); // Please remember we have incremented it
                                   // in the previous if
...
void
TipcSignalLinkTxBuffer::MergeItems (TipcSignalLinkTxItem *t1, TipcSignalLinkTxItem *t2) const
{
  NS_ASSERT (t1 != nullptr && t2 != nullptr);
  ...
  if (t1->m_retrans != t2->m_retrans)
    {
      if (t1->m_retrans)
        {
          TipcSignalLinkTxBuffer *self = const_cast<TipcSignalLinkTxBuffer*> (this);
...
void
TipcSignalLinkTxBuffer::RemoveFromCounts (TipcSignalLinkTxItem *item, uint32_t size)
{
...
bool
TipcSignalLinkTxBuffer::IsRetransmittedDataAcked (const SequenceNumber32& ack) const
{
  NS_LOG_FUNCTION (this);
...
void
TipcSignalLinkTxBuffer::DiscardUpTo (const SequenceNumber32& seq,
                          const Callback<void, TipcSignalLinkTxItem *> &beforeDelCb)
{
  NS_LOG_FUNCTION (this << seq);
  ...
  if (!m_sentList.empty ())
    {
      TipcSignalLinkTxItem *head = m_sentList.front ();
      if (head->m_sacked)
        {
...
uint32_t
TipcSignalLinkTxBuffer::Update (const TcpOptionSack::SackList &list,
                     const Callback<void, TipcSignalLinkTxItem *> &sackedCb)
{
  NS_LOG_FUNCTION (this);
...
void
TipcSignalLinkTxBuffer::UpdateLostCount ()
{
  NS_LOG_FUNCTION (this);
  uint32_t sacked = 0;
  ...
  if (sacked >= m_dupAckThresh)
    {
      TipcSignalLinkTxItem *item = *m_sentList.begin ();
      if (!item->m_lost)
        {
          item->m_lost = true;
          m_lostOut += item->m_packet->GetSize ();
...
bool
TipcSignalLinkTxBuffer::IsLost (const SequenceNumber32 &seq) const
{
  NS_LOG_FUNCTION (this << seq);
...
bool
TipcSignalLinkTxBuffer::NextSeg (SequenceNumber32 *seq, SequenceNumber32 *seqHigh, bool isRecovery) const
{
...
uint32_t
TipcSignalLinkTxBuffer::BytesInFlight () const
{
...
uint32_t
TipcSignalLinkTxBuffer::BytesInFlightRFC () const
{
  PacketList::const_iterator it;
  TipcSignalLinkTxItem *item;
...
bool
TipcSignalLinkTxBuffer::IsLostRFC (const SequenceNumber32 &seq, const PacketList::const_iterator &segment) const
{
  NS_LOG_FUNCTION (this << seq);
  uint32_t count = 0;
  uint32_t bytes = 0;
...
void
TipcSignalLinkTxBuffer::ResetRenoSack ()
{
  NS_LOG_FUNCTION (this);
...
void
TipcSignalLinkTxBuffer::SetRWndCallback (Callback<uint32_t> rWndCallback)
{
  NS_LOG_FUNCTION (this);
...
void
TipcSignalLinkTxBuffer::ResetSentList ()
{
  NS_LOG_FUNCTION (this);
  TipcSignalLinkTxItem *item;
...
void
TipcSignalLinkTxBuffer::ResetLastSegmentSent ()
{
  NS_LOG_FUNCTION (this);
  if (!m_sentList.empty ())
    {
      TipcSignalLinkTxItem *item = m_sentList.back ();
...
void
TipcSignalLinkTxBuffer::SetSentListLost (bool resetSack)
{
  NS_LOG_FUNCTION (this);
  m_retrans = 0;
...
bool
TipcSignalLinkTxBuffer::IsHeadRetransmitted () const
{
  NS_LOG_FUNCTION (this);
...
void
TipcSignalLinkTxBuffer::DeleteRetransmittedFlagFromHead ()
{
  NS_LOG_FUNCTION (this);
...
void
TipcSignalLinkTxBuffer::MarkHeadAsLost ()
{
  if (m_sentList.size () > 0)
    {
...
void
TipcSignalLinkTxBuffer::AddRenoSack (void)
{
  NS_LOG_FUNCTION (this);
...
void
TipcSignalLinkTxBuffer::ConsistencyCheck () const
{
...
std::ostream &
operator<< (std::ostream & os, TipcSignalLinkTxItem const & item)
{
  item.Print (os);
  return os;
}
...
std::ostream &
operator<< (std::ostream & os, TipcSignalLinkTxBuffer const & tcpTxBuf)
{
  TipcSignalLinkTxBuffer::PacketList::const_iterator it;
  std::stringstream ss;
